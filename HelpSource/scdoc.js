...
function addInheritedMethods() {
  ...
  if (!sups) return;
  var divs = [
    document.getElementById("inheritedclassmets"),
    document.getElementById("inheritedinstmets"),
  ];
  for (var i = 0; i < sups.length; i++) {
    ...
    for (var j = 0; j < mets.length; j++) {
      var m = mets[j];
      if (doc.methods.indexOf("_" + m.slice(1)) < 0) {
        // ignore methods already documented in this helpfile
...
function set_up_toc() {
  var toc_container = $("<div>", { id: "toc-container" }).insertBefore(
    $("#menubar").children().first(),
  );
...
function buildThemeSwitcher() {
  create_menubar_item("Theme \u25bc", "#", function (a, li) {
    const themesMenu = $("<div>", { class: "submenu" }).hide().appendTo(li);
...
function applyTheme(theme = null) {
  const themeName =
    theme ?? localStorage.getItem(scDocsThemeStorageKey) ?? "default";

  const cssThemeTag = document.getElementById("scdoc-theme");
  if (cssThemeTag === null) {
    console.log(
      `Could not find scdoc-theme css tag! Can not apply theme ${themeName}`,
    );
    return;
  }
